...
from history.models import  BrowsedUrlDetail
...
def main():
    # qs = BrowsedUrlDetail.objects.all()
    # qs_to_local_csv(qs, fields=['site', 'site_title','site_count', 'last_visit_time'])
    mark_qs = MarkedUrl.objects.all()
    qs_to_local_csv(mark_qs, fields=['url', 'title','created_at'])
